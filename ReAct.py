...
if __name__ == "__main__":
    ...
    with SqliteSaver.from_conn_string(":memory:") as memory:
        ...
        app.get_graph().draw_mermaid_png(output_file_path="graphs/graph_react.png")

        thread_id = "conversation_123"
        config = {"configurable": {"thread_id": thread_id}}

        response = app.invoke({"messages": ["What is the weather in London? And give me 10+20"]}, config=config)

        print(response['messages'][-1].content)

        response = app.invoke({"messages": ["What is the city I asked temperature for?"]}, config=config)
